...
class AccountController {
  ...
  async addAccountTransaction(req, res, next) {
    try {
      ...
      // INCREMENTAL BF UPDATE: Credit adds to BF, Debit subtracts from BF
      const netAmount = credit - debit;
      const bfResult = bfCalculation.adjustBFForAccount(lineId, netAmount);
  ...
  async deleteAccountTransaction(req, res, next) {
    try {
      ...
      // Get transaction amounts before deleting
      const deletedTransaction = transactions[transactionIndex];
      const credit = parseFloat(deletedTransaction.creditAmount) || 0;
      const debit = parseFloat(deletedTransaction.debitAmount) || 0;
      const netAmount = credit - debit;
      
      transactions = transactions.filter(t => t.id !== transactionId);
      fileManager.writeJSON(`account_transactions/${lineId}/${id}.json`, transactions);
      
      // INCREMENTAL BF UPDATE: Reverse the account transaction effect
      const bfResult = bfCalculation.adjustBFForAccount(lineId, -netAmount);
