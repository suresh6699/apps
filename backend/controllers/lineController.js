...
class LineController {
  // Get all lines with current BF
  async getAllLines(req, res, next) {
    try {
      let lines = fileManager.readJSON('lines.json') || [];

      // Return lines with their stored BF (no recalculation)
      lines = lines.map(line => {
        const currentBF = bfCalculation.getCurrentBF(line.id);
        return {
          ...line,
          currentBF: currentBF
        };
  ...
  async getLineById(req, res, next) {
    try {
      ...
      // Get current BF (no recalculation)
      const currentBF = bfCalculation.getCurrentBF(line.id);
  ...
  async getLineBF(req, res, next) {
    try {
      ...
      const currentBF = bfCalculation.getCurrentBF(id);
