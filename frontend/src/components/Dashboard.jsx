...
import { Plus, LogOut, Calendar, Edit, MoreVertical, Trash2, User as UserIcon } from 'lucide-react';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from './ui/dropdown-menu';
import ThemeSwitchFlowGlassPro from './ui/theme-switch-flow-glass-pro';
...
const Dashboard = () => {
  ...
  // Debug: Log user data on Dashboard mount and when user changes
  useEffect(() => {
    console.log('Dashboard - Current user:', user);
    console.log('Dashboard - User googleId:', user?.googleId);
    console.log('Dashboard - User picture:', user?.picture);
    console.log('Dashboard - User has Google connection:', !!user?.googleId);
  }, [user]);
  ...
  const loadLines = async () => {
    try {
      setLoading(true);
      const data = await lineService.getLines();
      setLines(data.lines || []);
    } catch (err) {
  ...
  const handleAddLine = async () => {
    ...
    try {
      ...
      if (editingLine) {
        await lineService.updateLine(editingLine.id, lineData);
      } else {
  ...
  const confirmDeleteLine = async () => {
    ...
    try {
      await lineService.deleteLine(lineToDelete.id);
      await loadLines();
              ...
              >
                ...
                <div className={`relative backdrop-blur-lg rounded-xl p-4 border shadow-lg hover:shadow-cyan-500/10 transition-all duration-300 group-hover:border-cyan-500/30 ${
                  ...
                  <div className={`mb-3 p-2.5 rounded-lg border ${
                    ...
                    }`}>₹{line.currentBF ? line.currentBF.toFixed(2) : '0.00'}</p>
                  </div>
